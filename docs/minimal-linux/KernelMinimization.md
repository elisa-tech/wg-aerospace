...
See [Hello World init](./HelloWorldInit.md) for how to build the statically
...
using NASA Core Flight System in [Hello cFS](./KernelMinimizationCFS.md).
...
[Certification progression](./CertificationProgression.md).
...
and the userspace init build in [Hello World init](./HelloWorldInit.md). On
Debian/Ubuntu:
...
- `cpio` — packs the initramfs in [Hello World init](./HelloWorldInit.md).
- `qemu-system-arm` — provides `qemu-system-aarch64` for booting the arm64
  `Image` (used in the walkthrough and in [Hello World init](./HelloWorldInit.md)).
...
> follow [Hello World init](./HelloWorldInit.md) to produce the
...
[Minimal Kernel Config Analysis](./KernelConfigAnalysis.md).
...
- **`kcov`** (`CONFIG_KCOV`**)** — per-task PC traces of the kernel paths a
  ...
  See [Hello World init → Dumping kernel coverage (KCOV)](./HelloWorldInit.md#dumping-kernel-coverage-kcov)
...
2. **Verification argument** — the activated-vs-present split maps onto the
   ...
   [Measuring Linux Kernel Complexity](./KernelComplexity.md) and
   [Certification progression](./CertificationProgression.md)).
...
- **Boots?** and **workload passes?** — the config is only valid if the target
  still runs (boot-test in QEMU as in [Hello World init](./HelloWorldInit.md)).

### Counting kernel SLOC

Use the [`sloc-counter`](./sloc-counter) script in this folder. It offers two
kernel counts:
...
[Hello World init](./HelloWorldInit.md) initramfs, then measure with
...
[Measuring Linux Kernel Complexity](./KernelComplexity.md) for build-time and
...
We count with the same [`sloc-counter`](./sloc-counter) script, using its
...
[Hello cFS](./KernelMinimizationCFS.md).

## References

### This walkthrough

- [`sloc-counter`](./sloc-counter) — the cloc-based SLOC script used for the
  Metrics tables (kernel/tree/files modes).
- [Hello World init](./HelloWorldInit.md) — build the minimal static PID 1 init
  and pack it into an initramfs for QEMU (the workload booted in this
  walkthrough).
- [Hello cFS](./KernelMinimizationCFS.md) — the same method applied to the NASA
  Core Flight System.
- [Certification progression](./CertificationProgression.md) — separate track on
  moving a minimized kernel toward higher assurance levels.
- [Measuring Linux Kernel Complexity](./KernelComplexity.md) — build-time and
  runtime complexity metrics (distinct from size).
- [Minimal Kernel Config Analysis](./KernelConfigAnalysis.md) — comparison
