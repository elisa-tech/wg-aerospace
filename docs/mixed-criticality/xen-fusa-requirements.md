...
| P-8 | Static shared memory for inter-partition communication, configured via device tree (not dom0-based libvchan)                                  | Deployment View |
...
| C-5 | A top-level device tree configures the hypervisor: partitions, scheduling, shared memory regions, and I/O assignments         | Deployment View, Logical View |
| C-6 | Per-partition device trees configure each guest domain's view of resources                                                    | Deployment View               |
...
| IPC-2 | Both configured via device tree (top-level for Xen, per-partition for guests) | Deployment View |
