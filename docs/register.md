<!--
SPDX-License-Identifier: CC-BY-SA-4.0
-->

# Document and information register

This document register shall help to find the information you are looking for. Documents are stored underneath the respective sub-project / folder. This gives an overview about all of them:

## Resources and papers

[Resources about ELISA, journals and articles / publications](./resources.md)
[Landing page for various paper ideas](./survey/README.md)

- [Paper ideas in discussions](https://github.com/elisa-tech/wg-aerospace/discussions?discussions_q=is%3Aopen%20label%3Apublications)

## Events

[Upcoming/Recent Events](./events.md)

## Product profiles

[Envisioned product profiles from space and aerospace](./product-profiles.md)

## Aerospace Use Cases

[Aerospace Use Cases landing page](./Aerospace-Uses-Cases.md)

- [Aerospace Use Cases - Cabin Lights](./use-cases/use-case-cabin-lights.md)
- [Aerospace Use Cases - Cabin Lights using NASA cFS](./use-cases/use-case-cabin-lights-cfs.md)
- [Use-case related items in discussions](https://github.com/elisa-tech/wg-aerospace/discussions?discussions_q=is%3Aopen%20label%3Ause-case)

## Demos (with respect to use-cases)

[Demos landing page](./../demos/Readme.md)

- [For users: Demo environment and dependencies](./../demos/docs/EnvSetup.md)
- [For developers: Environment setup](./../demos/docs/Development.md)
  - [Specific: Build The Core Flight System (cFS)](./../demos/docs/Build-cFS.md)
  - [Specific: Modify Root-FS CPIO](./../demos/docs/ModifyRootfsCpio.md)
  - [Specific: Environment building notes](./../demos/env/Readme.md)

[Copilot landing page](./../demos/copilot/Readme.md)

- [Copilot native test app and Copilot walk through](./../demos/copilot/BasicDemo.md)
- [Copilot embedded demo](./../demos/copilot/EmbeddedDemo.md)

[AvioNix Demo](./../demos/AvioNix-demo/README.md)

## Embedded environments (with respect to use-cases)

[Embedded environments landing page](./../demos/docs/embedded-environments/Readme.md)

- [Stand-alone embedded environments](./../demos/docs/embedded-environments/Standalone.md)
- [Mixed functions embedded environments](./../demos/docs/embedded-environments/MixedFunctions.md)
- Host Dependencies (e.g., environment demos run on)
  - [Container building](./../demos/env/Readme.md)
  - [Host system testing](./../demos/env/test/host-ubuntu/README.md)
