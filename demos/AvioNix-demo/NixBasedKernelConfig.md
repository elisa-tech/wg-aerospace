...
The goal of this document is to provide means to change the kernel config to ean extend that it gets minimal to your needs on an abstraction level hight than the actual kernel config file.
There are many more dimensions to it like target hardware and/or kernel version, but this is a starting point.
Switching on/off certain features of the kernel this way ensures that also dependnecies are covered and reduced the amount of parameters to a minimum in the first place.
...
4.  To actually make kernel config out of this meta-conig, you can build the actual kernel config file with this command:
