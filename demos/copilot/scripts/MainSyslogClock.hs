...
checkOfMonitor1 :: Stream Bool
checkOfMonitor1 = lightsOn
        && timeSinceLastTime' switchTurnedOn > timeSinceLastTime lightsOn
        && timeSinceLastTime' switchTurnedOn < limit
...
spec = do
  ...
  -- trigger "noViolation1" checkOfMonitor1 [ arg (clock - timeSinceLastTime switchTurnedOn) ]
