...
def run_qemu_with_watch(str_to_exit_on, timeout, commands):
    qemu_command = [
        ...
        "Image",
        "-append",
        "root=/dev/ram0 init=/bin/dash console=ttyAMA0 loglevel=7",
        "-initrd",
        "rootfs.cpio.gz_new",
    ]
        ...
        if qemu_process.stdout in ready:
            ...
            if resultStart:
                if outputFile is None:
                    outputFile = open(
                        "../../../test-results/test-results-system-cross.xml", "w"
                    )
                if str_to_exit_on not in output:
                    outputFile.write(output)

            if isWatchedStr(
                "Test results directory not found. Writing results to console", output
            ):
                resultStart = True
