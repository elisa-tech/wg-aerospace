...
class ServerCommandBase(ABC):
    ...
    def DeSerialize(data):
        """Function that unpack data from a raw utf-8 buffer into an object."""
        pass
...
class ServerCommandJson(ServerCommandBase):
    ...
    def Serialize(self):
        ...
        # we temporarily replace the enums with the corresponding integer values and restore them after
    ...
    def DeSerialize(data):
        ...
        return ServerCommandJson(
            Devices(jsonObj["DeviceID"]), ServerCommands(jsonObj["Command"])
        )
...
class ServerCommandLightText(ServerCommandBase):
    ...
    def Serialize(self):
        ...
        command = (
            ServerCommandLightText.CommandPrefix
            + ServerCommandLightText.CommandSeparator
            + ServerCommandLightText.OnCommand
            if self.Command == ServerCommands.TURN_ON_DEVICE
            else ServerCommandLightText.CommandPrefix
            + ServerCommandLightText.CommandSeparator
            + ServerCommandLightText.OffCommand
        )
    ...
    def DeSerialize(data):
        ...
        command = data.decode("utf-8").split(
            sep=ServerCommandLightText.CommandSeparator, maxsplit=1
        )
        if len(command) == 2 and command[0] == ServerCommandLightText.CommandPrefix:
            # This means that this is a light command
            returnValue = ServerCommandLightText(
                Devices.LIGHT,
                (
                    ServerCommands.TURN_OFF_DEVICE
                    if command[1] == ServerCommandLightText.OffCommand
                    else ServerCommands.TURN_ON_DEVICE
                ),
            )
        return returnValue
