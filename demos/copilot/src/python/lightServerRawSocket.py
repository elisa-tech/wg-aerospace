...
class LightServerDelay(object):
    ...
    def SetDelayRandom(self, delayTimeLowerLimitMs, delayTimeUpperLimitMs):
        """Turn off Delay"""
        self.logger.info(
            f"Setting delay to Random - {delayTimeLowerLimitMs} ms to {delayTimeUpperLimitMs} ms"
        )
        with self.LockObj:
    ...
    def DoDelay(self):
        ...
        with self.LockObj:
            ...
            if ServerDelayType.OFF == self.DelayControlSetting:
                ...
                delayTimeSec = (
                    random.randrange(
                        self.RandomDelayLowerLimitMs,
                        self.RandomDelayUpperLimitMs,
                        self.RandomDelayIncrement,
                    )
                    / 1000
                )
...
class LightServer(threading.Thread):
    ...
    def __init__(self, delayController):
        ...
        # This eth type is meant to be used for local experimentation
        # Refer https://www.iana.org/assignments/ieee-802-numbers/ieee-802-numbers.xhtml

        self.ethType = 0x88B5
        self.running = True
        self.serverAddress = appConfiguration.ServerConfiguration.ETHERNET_INTERFACE
        self.sock = socket.socket(
            socket.AF_PACKET, socket.SOCK_RAW, socket.htons(self.ethType)
        )
        self.sock.setblocking(False)
        self.sock.bind((self.serverAddress, 0))
    ...
    def run(self):
        ...
        while self.running:
            try:
                commandData = self.sock.recv(4096)
                # For now, don't check the mac address. We use Eth Type field to identify that the
                # packet is meant for us.
                dest_mac = commandData[:6]
                src_mac = commandData[6:12]
                frame_type = struct.unpack("!H", commandData[12:14])[0]

                if frame_type == self.ethType:
                    commandData = commandData[14:]
                    # Parse the command only if it is meant for the server.
...
class ServerMain:
    """Main application that is responsible for kicking off the server thread and monitoring signals."""
    ...
    def _parseCommand(self, inputLine):
        ...
        if fixedCommandMatch:
            ...
            self.DelayController.SetDelayRandom(
                int(randomCommandMatch[1]), int(randomCommandMatch[2])
            )
    ...
    def Start(self):
        ...
        try:
            print(
                "Enter Commands (fixed <delay>, random <low> <high>, higher or lower)."
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Light Server program with configurable delay"
    )

    delay_group = parser.add_mutually_exclusive_group()
    delay_group.add_argument(
        "--delay-fixed", type=int, help="Fixed delay time in millisecond"
    )
    delay_group.add_argument(
        "--delay-random",
        type=int,
        nargs=2,
        metavar=("lowerLimit", "upperLimit"),
