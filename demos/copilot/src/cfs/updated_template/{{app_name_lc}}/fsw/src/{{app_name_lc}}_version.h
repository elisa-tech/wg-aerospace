...
#define{{app_name_uc } } _BUILD_BASELINE                                       \
    "equuleus-rc1" /*!< Development Build: git tag that is the base for the    \
                      current development */
#define{{app_name_uc } } _BUILD_DEV_CYCLE                                      \
    "equuleus-rc2" /**< @brief Development: Release name for current           \
                      development cycle */
#define{{app_name_uc } } _BUILD_CODENAME                                       \
...
#define{{                                                                      \
    app_name_uc } } _REVISION 0 /*!< @brief Revision version number. Value of  \
                                   0 indicates a development version.*/
...
#define{{app_name_uc } } _STR_HELPER(                                          \
    x) #x /*!< @brief Helper function to concatenate strings from integer      \
             macros */
#define{{app_name_uc } } _STR(x){{                                             \
    app_name_uc } } _STR_HELPER(x) /*!< @brief Helper function to concatenate  \
                                      strings from integer macros */
 ...
 * versions.
 */
#define{{app_name_uc } } _VERSION{{app_name_uc } } _BUILD_BASELINE             \
    "+dev" {{app_name_uc } } _STR({{app_name_uc}} _BUILD_NUMBER)
