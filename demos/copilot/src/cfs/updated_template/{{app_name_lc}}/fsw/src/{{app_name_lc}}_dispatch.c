  ...
  */
  ...
  case { { app_name_uc } } _NOOP_CC:
    if ({
        ...
        } _VerifyCmdLength(&SBBufPtr->Msg, sizeof({
          {
            app_name_uc
          }
        } _NoopCmd_t))) {
  ...
  default:
    CFE_EVS_SendEvent(
        ...
        CFE_EVS_EventType_ERROR, "Invalid ground command code: CC = %d",
        CommandCode);
    break;
  }
}
...
_TaskPipe(const CFE_SB_Buffer_t *SBBufPtr) {
  ...
  case { { app_name_uc } } _CMD_MID: {
    ...
    _ProcessGroundCommand(SBBufPtr);
    break;
...
void{{triggerName}}({
  ...
  void{{triggerName}}(void) {
    ...
    CFE_EVS_SendEvent(
        ...
        CFE_EVS_EventType_ERROR, "COPILOT: violation: {{triggerName}}");
  }
  {
    {
      / triggers
