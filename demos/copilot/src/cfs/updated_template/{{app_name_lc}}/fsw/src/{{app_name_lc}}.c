...
_Main(void) {
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    ...
    _Data.RunStatus = CFE_ES_RunStatus_APP_ERROR;
  }
...
_Init(void) {
  ...
  _Data.RunStatus = CFE_ES_RunStatus_APP_RUN;
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "{{app_name_txt}} App: Error Registering Events, RC = 0x%08lX\n",
        (unsigned long)status);
  } else {
    /*
     ** Initialize housekeeping packet (clear user data area).
     */
    CFE_MSG_Init(CFE_MSG_PTR({
                   {
                     app_name_uc
                   }
    ...
    if (status != CFE_SUCCESS) {
      CFE_EVS_SendEvent(
          ...
          CFE_EVS_EventType_ERROR,
          "{{app_name_txt}} App: Error creating SB Command Pipe, RC = 0x%08lX",
          (unsigned long)status);
    }
  }
    ...
    */
    ...
    if (status != CFE_SUCCESS) {
      CFE_EVS_SendEvent(
          ...
          CFE_EVS_EventType_ERROR,
          "{{app_name_txt}} App: Error Subscribing to HK request, RC = 0x%08lX",
          (unsigned long)status);
    }
  }
    ...
    */
    ...
    if (status != CFE_SUCCESS) {
      CFE_EVS_SendEvent(
          ...
          CFE_EVS_EventType_ERROR,
          "{{app_name_txt}} App: Error Subscribing to Commands, RC = 0x%08lX",
          (unsigned long)status);
    }
  }
    ...
    */
    ...
    if (status != CFE_SUCCESS) {
      CFE_EVS_SendEvent(
          ...
          CFE_EVS_EventType_ERROR,
          "{{app_name_txt}} App: Error Registering Example Table, RC = 0x%08lX",
          (unsigned long)status);
    } else {
      status = CFE_TBL_Load(
          {
    ...
    CFE_EVS_SendEvent(
        ...
        CFE_EVS_EventType_INFORMATION, "{{app_name_txt}} App Initialized.%s",
        VersionString);
  }
