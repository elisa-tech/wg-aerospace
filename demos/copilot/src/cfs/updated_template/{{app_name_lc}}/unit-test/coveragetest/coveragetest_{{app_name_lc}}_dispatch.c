...
_ProcessGroundCommand(void) {
  ...
  union {
    ...
    _ResetCountersCmd_t Reset;
    {
      {
        app_name_uc
      }
    }
    _ProcessCmd_t Process;
    {
      {
        app_name_uc
      }
    }
    _DisplayParamCmd_t DisplayParam;
  } TestMsg;
  UT_CheckEvent_t EventTest;
  ...
  _ProcessGroundCommand(&TestMsg.SBBuf);
  ...
  /* note this will end up calling {{app_name_uc}}_Process(), and as such it
   * needs to avoid dereferencing a table which does not exist. */
