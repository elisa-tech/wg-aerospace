...
void Test_LIGHTS_APP_TaskPipe(void)
{
     ...
     */
    TestMsgId = CFE_SB_ValueToMsgId(LIGHTS_APP_CMD_MID);
    FcnCode   = LIGHTS_APP_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    LIGHTS_APP_TaskPipe(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(LIGHTS_APP_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    LIGHTS_APP_TaskPipe(&TestMsg.SBBuf);
...
void Test_LIGHTS_APP_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void LIGHTS_APP_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t               SBBuf;
        LIGHTS_APP_NoopCmd_t          Noop;
        LIGHTS_APP_ResetCountersCmd_t Reset;
        LIGHTS_APP_ProcessCmd_t       Process;
        LIGHTS_APP_DisplayParamCmd_t  DisplayParam;
    } TestMsg;
    UT_CheckEvent_t EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, LIGHTS_APP_CMD_LEN_ERR_EID,
    ...
    LIGHTS_APP_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_STUB_COUNT(LIGHTS_APP_NoopCmd, 1);
    ...
    FcnCode = LIGHTS_APP_RESET_COUNTERS_CC;
    ...
    UtAssert_STUB_COUNT(LIGHTS_APP_ResetCountersCmd, 1);
    ...
    /* note this will end up calling LIGHTS_APP_Process(), and as such it needs to
     * avoid dereferencing a table which does not exist. */
    FcnCode = LIGHTS_APP_PROCESS_CC;
    ...
    UtAssert_STUB_COUNT(LIGHTS_APP_ProcessCmd, 1);
    ...
    FcnCode = LIGHTS_APP_DISPLAY_PARAM_CC;
    ...
    UtAssert_STUB_COUNT(LIGHTS_APP_DisplayParamCmd, 1);
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CHECKEVENT_SETUP(&EventTest, LIGHTS_APP_CC_ERR_EID, "Invalid ground command code: CC = %d");
    LIGHTS_APP_ProcessGroundCommand(&TestMsg.SBBuf);
...
void Test_LIGHTS_APP_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool LIGHTS_APP_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    ...
    LIGHTS_APP_VerifyCmdLength(NULL, size);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    LIGHTS_APP_VerifyCmdLength(NULL, size + 1);
...
void UtTest_Setup(void)
{
    ...
    ADD_TEST(LIGHTS_APP_ProcessGroundCommand);
    ADD_TEST(LIGHTS_APP_VerifyCmdLength);
}
