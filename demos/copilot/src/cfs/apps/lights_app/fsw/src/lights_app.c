    ...
    */
    while (CFE_ES_RunLoop(&LIGHTS_APP_Data.RunStatus) == true)
    {
        ...
        */
        ...
        status = CFE_SB_ReceiveBuffer(&SBBufPtr, LIGHTS_APP_Data.CommandPipe, CFE_SB_PEND_FOREVER);
...
CFE_Status_t LIGHTS_APP_Init(void)
{
    ...
    */
    ...
    else
    {
         ...
         */
        CFE_MSG_Init(CFE_MSG_PTR(LIGHTS_APP_Data.HkTlm.TelemetryHeader), CFE_SB_ValueToMsgId(LIGHTS_APP_HK_TLM_MID),
                     sizeof(LIGHTS_APP_Data.HkTlm));

        /*
         ** Create Software Bus message pipe.
         */
        status = CFE_SB_CreatePipe(&LIGHTS_APP_Data.CommandPipe, LIGHTS_APP_PIPE_DEPTH, LIGHTS_APP_PIPE_NAME);
        if (status != CFE_SUCCESS)
        {
    ...
    if (status == CFE_SUCCESS)
    {
        ...
        */
        status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(LIGHTS_APP_SEND_HK_MID), LIGHTS_APP_Data.CommandPipe);
        if (status != CFE_SUCCESS)
        {
    ...
    if (status == CFE_SUCCESS)
    {
        ...
        */
        status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(LIGHTS_APP_CMD_MID), LIGHTS_APP_Data.CommandPipe);
        if (status != CFE_SUCCESS)
        {
    ...
    if (status == CFE_SUCCESS)
    {
        ...
        */
        status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(LIGHTS_COMMAND), LIGHTS_APP_Data.CommandPipe);
        if (status != CFE_SUCCESS)
        {
            CFE_EVS_SendEvent(LIGHTS_APP_SUB_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
                              "Lights App: Error Subscribing to Commands, RC = 0x%08lX", (unsigned long)status);
    ...
    if (status == CFE_SUCCESS)
    {
        ...
        */
        status = CFE_TBL_Register(&LIGHTS_APP_Data.TblHandles[0], "ExampleTable", sizeof(LIGHTS_APP_ExampleTable_t),
                                  CFE_TBL_OPT_DEFAULT, LIGHTS_APP_TblValidationFunc);
        if (status != CFE_SUCCESS)
        {
