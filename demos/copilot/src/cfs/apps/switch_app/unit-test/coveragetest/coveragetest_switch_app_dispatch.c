...
void Test_SWITCH_APP_TaskPipe(void)
{
     ...
     */
    TestMsgId = CFE_SB_ValueToMsgId(SWITCH_APP_CMD_MID);
    FcnCode   = SWITCH_APP_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    SWITCH_APP_TaskPipe(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(SWITCH_APP_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    SWITCH_APP_TaskPipe(&TestMsg.SBBuf);
...
void Test_SWITCH_APP_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void SWITCH_APP_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t               SBBuf;
        SWITCH_APP_NoopCmd_t          Noop;
        SWITCH_APP_ResetCountersCmd_t Reset;
        SWITCH_APP_ProcessCmd_t       Process;
        SWITCH_APP_DisplayParamCmd_t  DisplayParam;
    } TestMsg;
    UT_CheckEvent_t EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, SWITCH_APP_CMD_LEN_ERR_EID,
    ...
    SWITCH_APP_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_STUB_COUNT(SWITCH_APP_NoopCmd, 1);
    ...
    FcnCode = SWITCH_APP_RESET_COUNTERS_CC;
    ...
    UtAssert_STUB_COUNT(SWITCH_APP_ResetCountersCmd, 1);
    ...
    /* note this will end up calling SWITCH_APP_Process(), and as such it needs to
     * avoid dereferencing a table which does not exist. */
    FcnCode = SWITCH_APP_PROCESS_CC;
    ...
    UtAssert_STUB_COUNT(SWITCH_APP_ProcessCmd, 1);
    ...
    FcnCode = SWITCH_APP_DISPLAY_PARAM_CC;
    ...
    UtAssert_STUB_COUNT(SWITCH_APP_DisplayParamCmd, 1);
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CHECKEVENT_SETUP(&EventTest, SWITCH_APP_CC_ERR_EID, "Invalid ground command code: CC = %d");
    SWITCH_APP_ProcessGroundCommand(&TestMsg.SBBuf);
...
void Test_SWITCH_APP_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool SWITCH_APP_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    ...
    SWITCH_APP_VerifyCmdLength(NULL, size);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    SWITCH_APP_VerifyCmdLength(NULL, size + 1);
...
void UtTest_Setup(void)
{
    ...
    ADD_TEST(SWITCH_APP_ProcessGroundCommand);
    ADD_TEST(SWITCH_APP_VerifyCmdLength);
}
