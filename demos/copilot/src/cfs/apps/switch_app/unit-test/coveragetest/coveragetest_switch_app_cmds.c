...
void Test_SWITCH_APP_ReportHousekeeping(void) {
  /*
   * Test Case For:
   * void SWITCH_APP_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
   */
   ...
   */
  SWITCH_APP_SendHkCmd(NULL);

  /* Confirm message sent*/
  UtAssert_STUB_COUNT(CFE_SB_TransmitMsg, 1);
  UtAssert_ADDRESS_EQ(MsgSend, &SWITCH_APP_Data.HkTlm);

  /* Confirm timestamp msg address */
  UtAssert_STUB_COUNT(CFE_SB_TimeStampMsg, 1);
  UtAssert_ADDRESS_EQ(MsgTimestamp, &SWITCH_APP_Data.HkTlm);
...
void Test_SWITCH_APP_NoopCmd(void) {
  /*
   * Test Case For:
   * void SWITCH_APP_NoopCmd( const SWITCH_APP_Noop_t *Msg )
   */
  SWITCH_APP_NoopCmd_t TestMsg;
  UT_CheckEvent_t EventTest;

  memset(&TestMsg, 0, sizeof(TestMsg));

  /* test dispatch of NOOP */
  UT_CHECKEVENT_SETUP(&EventTest, SWITCH_APP_NOOP_INF_EID, NULL);

  UtAssert_INT32_EQ(SWITCH_APP_NoopCmd(&TestMsg), CFE_SUCCESS);
...
void Test_SWITCH_APP_ResetCountersCmd(void) {
  /*
   * Test Case For:
   * void SWITCH_APP_ResetCounters( const SWITCH_APP_ResetCounters_t *Msg )
   */
  SWITCH_APP_ResetCountersCmd_t TestMsg;
  UT_CheckEvent_t EventTest;

  memset(&TestMsg, 0, sizeof(TestMsg));

  UT_CHECKEVENT_SETUP(&EventTest, SWITCH_APP_RESET_INF_EID,
...
void Test_SWITCH_APP_ProcessCmd(void) {
  /*
   * Test Case For:
   * void  SWITCH_APP_ProcessCmd( const SWITCH_APP_ProcessCmd_t *Msg )
   */
  SWITCH_APP_ProcessCmd_t TestMsg;
  ...
  /* Provide some table data for the SWITCH_APP_Process() function to use */
  TestTblData.Int1 = 40;
  TestTblData.Int2 = 50;
  UT_SetDataBuffer(UT_KEY(CFE_TBL_GetAddress), &TblPtr, sizeof(TblPtr), false);
  UtAssert_INT32_EQ(SWITCH_APP_ProcessCmd(&TestMsg), CFE_SUCCESS);
   ...
   * SWITCH_APP_GetCrc() function is a stub.
   */
  ...
  UtAssert_INT32_EQ(SWITCH_APP_ProcessCmd(&TestMsg), CFE_TBL_ERR_UNREGISTERED);
  ...
  UT_SetDefaultReturnValue(UT_KEY(CFE_TBL_ReleaseAddress),
                           CFE_TBL_ERR_NO_ACCESS);
  UtAssert_INT32_EQ(SWITCH_APP_ProcessCmd(&TestMsg), CFE_TBL_ERR_NO_ACCESS);
  UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 4);
}

void Test_SWITCH_APP_DisplayParamCmd(void) {
  /*
   * Test Case For:
   * void  SWITCH_APP_DisplayParamCmd( const SWITCH_APP_DisplayParamCmd_t *Msg )
   */
  SWITCH_APP_DisplayParamCmd_t TestMsg;
  UT_CheckEvent_t EventTest;

  memset(&TestMsg, 0, sizeof(TestMsg));

  UT_CHECKEVENT_SETUP(&EventTest, SWITCH_APP_VALUE_INF_EID,
                      "SWITCH_APP: ValU32=%lu, ValI16=%d, ValStr=%s");
  TestMsg.Payload.ValU32 = 10;
  TestMsg.Payload.ValI16 = -4;
  snprintf(TestMsg.Payload.ValStr, sizeof(TestMsg.Payload.ValStr), "Hello");

  UtAssert_INT32_EQ(SWITCH_APP_DisplayParamCmd(&TestMsg), CFE_SUCCESS);
  /*
   * Confirm that the event was generated
   */
  UtAssert_UINT32_EQ(EventTest.MatchCount, 1);
...
void UtTest_Setup(void) {
  ADD_TEST(SWITCH_APP_ReportHousekeeping);
  ADD_TEST(SWITCH_APP_NoopCmd);
  ADD_TEST(SWITCH_APP_ResetCountersCmd);
  ADD_TEST(SWITCH_APP_ProcessCmd);
  ADD_TEST(SWITCH_APP_DisplayParamCmd);
}
