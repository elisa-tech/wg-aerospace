...
lights :: Stream (Bool)
lights = extern "lights" Nothing

lightSwitch :: Stream (Bool)
lightSwitch = extern "lightSwitch" Nothing

-- | TestOgma
--   @
--   lights on if and only if lightSwitch is on
--   @
propTestOgma :: Stream Bool
propTestOgma = lights == lightSwitch
...
spec :: Spec
spec = do
  trigger "handlerTestOgma" (not propTestOgma) []

main :: IO ()
main = reify spec >>= compileWith settings "copilot"
  where
